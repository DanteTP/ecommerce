...
module.exports = {
  ...
  cartlogin: (req,res,next)=>{
    if(req.user!==''){
      res.render('checkout',{title:'express',user:req.user,errors:'false',screen:'home',usererrors:'false',passerrors:'false',addresserror:'false',data:'false',imageerror:'false',paypal:process.env.CLIENT_ID})
  } else
  ...
  carcheckoutscreen: async (req,res,next) =>{
    ...
    if(req.body.cookie){ 
        ...
        res.render('checkout',{title:'express',user:user,errors:'false',screen:'home',usererrors:'false',passerrors:'false',addresserror:'false',data:'false',imageerror:'false',paypal:process.env.CLIENT_ID})
    }else{
        let user = await db.Users.findOne({where:{Email: req.body.Email},include:['direccionesusuario','imagenusuario']})
        user.Password=''
